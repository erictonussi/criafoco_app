appControllers.controller('TabScriptCtrl', ['$scope', '$rootScope', '$state', '$ionicPopup', '$translate', 'localStorageService', 'tipoRegistroManager', 'registroManager', 'projetoManager', function($scope, $rootScope, $state, $ionicPopup, $translate, localStorageService, tipoRegistroManager, registroManager, projetoManager) {
        ...
        tipoRegistroManager.getByFlag('note').then(function(response) {
            ...
            if (noteType != null) {
                registroManager.getAll($rootScope.projeto, noteType, false).then(function(response) {
                    $scope.oportunidades = response.filter(x => x.ob_op == 1);
