  ...
  .controller('TabNotesCtrl', function ($scope, $rootScope, $state, $timeout, $ionicModal, $ionicPopup, $ionicListDelegate, $translate, localStorageService, tipoRegistroManager, registroManager, projetoManager, ga) {
    ...
    function refreshList () {
      registroManager.getAll($rootScope.projeto, type, false).then(function (response) {
        ...
        var oportunidades = response.filter(function (x) {
          return x.ob_op === 1;
        });
        var obstaculos = response.filter(function (x) {
          return x.ob_op === 2;
        });
    ...
    $scope.scoreItem = function (record, event) {
      ...
      var relevancePopup = $ionicPopup.alert({
        ...
        template: '<p>{{current.descricao}}</p><div class="custom-range"><powerange ng-model="current.avaliacao"></powerange><strong>{{current.avaliacao > -1 ? current.avaliacao : "0"}}</strong></div>',
        scope: $scope,
        buttons: [
          { text: strings.cancel },
          {
    ...
    $scope.process = function () {
      ...
      if ($scope.step === 'note') {
        ...
        if (canContinue) {
          ...
          var itemsOp = Array.from({length: 11}, function (/*v, k*/) {
            return [];
          });
          ...
          // bestVotesOp.push(...itemsOp[i]);
            bestVotesOp.push.apply(bestVotesOp, itemsOp[i]);
          ...
          var itemsOb = Array.from({length: 11}, function (/*v, k*/) {
            return [];
          });
          ...
          for (i = itemsOb.length; i--;) {
          // bestVotesOb.push(...itemsOb[i]);
            bestVotesOb.push.apply(bestVotesOb, itemsOb[i]);
            ...
            $ionicPopup.alert({
              ...
              .then(function (/*res*/) {
                oportunidades.forEach(function (element/*, index*/) {
                  var found = bestVotesOp.find(function (x) {
                    return x.id === element.id;
                  } );
            ...
            $ionicPopup.alert({
              ...
              .then(function (/*res*/) {
                obstaculos.forEach(function (element/*, index*/) {
                  var found = bestVotesOb.find(function (x) {
                    return x.id === element.id;
                  } );
    ...
    $scope.obOp = function (record, value) {
      ...
      registroManager.save(record).then(function (/*response*/) {
        var index = $scope.records.findIndex(function (x) {
