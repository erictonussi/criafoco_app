appControllers.controller('TabCreationCtrl', ['$scope', '$rootScope', '$state', '$timeout', '$interval', '$ionicModal', '$ionicPopup', '$ionicPopover', '$translate', 'localStorageService', 'tipoRegistroManager', 'registroManager', 'projetoManager', function($scope, $rootScope, $state, $timeout, $interval, $ionicModal, $ionicPopup, $ionicPopover, $translate, localStorageService, tipoRegistroManager, registroManager, projetoManager) {
    ...
    $scope.process = function() {
        if ($scope.step == 'creation') {
            ...
            if (canContinue) {
                const items = Array.from({length: 11}, (v, k) => []);
        ...
        tipoRegistroManager.getByFlag('note').then(function(response) {
            ...
            if (noteType != null) {
                registroManager.getAll($rootScope.projeto, noteType, false).then(function(response) {
                    $scope.obstaculos = response.filter(x => x.ob_op == 2);
