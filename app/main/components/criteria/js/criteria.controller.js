appControllers.controller('CriteriaCtrl', ['$scope', '$rootScope', '$ionicPopup', '$translate', 'criterioManager', 'Criterio', function($scope, $rootScope, $ionicPopup, $translate, criterioManager, Criterio) {
    var strings = {};
    ...
    $scope.remover = function(item, event) {
        criterioManager.remove(item).then(function(response) {
            var index = $scope.criterios.findIndex(x => x.id == item.id);
